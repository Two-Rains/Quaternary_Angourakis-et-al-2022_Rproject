...
#' @export
watbal.update = function(modelVars, lastDay, param, typeOfParValue){
  FC = param[typeOfParValue, "FC"]
  WHC = param[typeOfParValue, "WHC"]
  WP = param[typeOfParValue, "WP"]
...
#' @export
watbal.model = function(param, weather, WAT0 = NA, typeOfParValue = "nominal")
{
  z = param[typeOfParValue, "z"]
  FC = param[typeOfParValue, "FC"]
  ...
  {
    modelVars <- watbal.update(modelVars, day, param, typeOfParValue)
  }
...
watbal.define.param = function()
{
  ...
  param = data.frame(FC, WHC, WP, MUF, DC, z, CN);
  row.names(param) = c("nominal","binf","bsup");
  param = as.matrix(param)
  attributes(param)$description=t(t(c(
    "WP" = "Wilting Point", 
